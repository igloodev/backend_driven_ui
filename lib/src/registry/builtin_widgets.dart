...
class BuiltinWidgets {
  static Map<String, Widget Function(WidgetSchema, BuildContext)> getBuilders(
    ...
    return {
      ...
      'CircularProgressIndicator': (s, c) =>
          DisplayBuilders.buildCircularProgressIndicator(s, c),
      'LinearProgressIndicator': (s, c) =>
          DisplayBuilders.buildLinearProgressIndicator(s, c),
      ...
      'CircleAvatar': (s, c) =>
          MaterialBuilders.buildCircleAvatar(s, c, parser),
      'Chip': (s, c) => MaterialBuilders.buildChip(s, c),
      'ClipRRect': (s, c) => MaterialBuilders.buildClipRRect(s, c, parser),
      'FloatingActionButton': (s, c) =>
          MaterialBuilders.buildFloatingActionButton(s, c, parser),
      'ExpansionTile': (s, c) =>
          MaterialBuilders.buildExpansionTile(s, c, parser),
      ...
      // Interactive
      'Dismissible': (s, c) =>
          InteractiveBuilders.buildDismissible(s, c, parser),
      'Button': (s, c) => InteractiveBuilders.buildButton(s, c, parser),
      'ElevatedButton': (s, c) =>
          InteractiveBuilders.buildButton(s, c, parser, buttonType: 'elevated'),
      'TextButton': (s, c) =>
          InteractiveBuilders.buildButton(s, c, parser, buttonType: 'text'),
      'OutlinedButton': (s, c) =>
          InteractiveBuilders.buildButton(s, c, parser, buttonType: 'outlined'),
      'IconButton': (s, c) => InteractiveBuilders.buildIconButton(s, c, parser),
      'GestureDetector': (s, c) =>
          InteractiveBuilders.buildGestureDetector(s, c, parser),
      ...
      'ListView.builder': (s, c) =>
          ScrollableBuilders.buildListView(s, c, parser),
      'ListView.separated': (s, c) =>
          ScrollableBuilders.buildListViewSeparated(s, c, parser),
      'ListView.custom': (s, c) =>
          ScrollableBuilders.buildListViewCustom(s, c, parser),

      // Scrollable — GridView variants
      'GridView': (s, c) => ScrollableBuilders.buildGridView(s, c, parser),
      'GridView.builder': (s, c) =>
          ScrollableBuilders.buildGridView(s, c, parser),
      'GridView.count': (s, c) =>
          ScrollableBuilders.buildGridViewCount(s, c, parser),
      'GridView.extent': (s, c) =>
          ScrollableBuilders.buildGridViewExtent(s, c, parser),
      'GridView.custom': (s, c) =>
          ScrollableBuilders.buildGridViewCustom(s, c, parser),

      'SingleChildScrollView': (s, c) =>
          ScrollableBuilders.buildSingleChildScrollView(s, c, parser),

      // Slivers
      'CustomScrollView': (s, c) =>
          SliverBuilders.buildCustomScrollView(s, c, parser),
      ...
      'SliverToBoxAdapter': (s, c) =>
          SliverBuilders.buildSliverToBoxAdapter(s, c, parser),
      'SliverPadding': (s, c) =>
          SliverBuilders.buildSliverPadding(s, c, parser),
      'SliverFillRemaining': (s, c) =>
          SliverBuilders.buildSliverFillRemaining(s, c, parser),
      'SliverFixedExtentList': (s, c) =>
          SliverBuilders.buildSliverFixedExtentList(s, c, parser),

      // Navigation
      'BottomNavigationBar': (s, c) =>
          NavigationBuilders.buildBottomNavigationBar(s, c, parser),
      'NavigationBar': (s, c) =>
          NavigationBuilders.buildNavigationBar(s, c, parser),
      'DefaultTabController': (s, c) =>
          NavigationBuilders.buildDefaultTabController(s, c, parser),
      ...
      'PageView.builder': (s, c) =>
          PageViewBuilders.buildPageViewBuilder(s, c, parser),

      // Cupertino (iOS-native)
      'CupertinoButton': (s, c) =>
          CupertinoBuilders.buildCupertinoButton(s, c, parser),
      'CupertinoSwitch': (s, c) =>
          CupertinoBuilders.buildCupertinoSwitch(s, c, parser),
      'CupertinoSlider': (s, c) =>
          CupertinoBuilders.buildCupertinoSlider(s, c, parser),
      'CupertinoActivityIndicator': (s, c) =>
          CupertinoBuilders.buildCupertinoActivityIndicator(s, c),
      'CupertinoTextField': (s, c) =>
          CupertinoBuilders.buildCupertinoTextField(s, c, parser),
